      ...
      <div className="relative max-w-[1600px] mx-auto p-6 space-y-8">
        <div>
          <h1 className="text-3xl font-black tracking-tight flex items-center gap-3">
            ...
            <span className="px-3 py-1 rounded-full bg-crypto-accent/10 border border-crypto-accent/20 text-crypto-accent text-xs font-bold tracking-widest">4 MODELS + ENSEMBLE</span>
          </h1>
          <p className="text-crypto-muted text-sm mt-2">End-to-end crypto forecasting pipeline • 69 features • Sentiment • Real-time • Backtesting</p>
        </div>
              ...
              <div className="flex items-start justify-between mb-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <div className="text-xs font-bold tracking-widest text-crypto-muted uppercase">{model.subtitle}</div>
                  </div>
                </div>
                <div className="text-right">
                  <div className="text-xs font-bold tracking-widest text-crypto-muted uppercase">Performance</div>
              ...
              <div className="space-y-2">
                {model.specs.map((spec, i) => (
                  <div key={i} className="flex items-center gap-2 text-xs">
                    ...
                    <span className="mono text-crypto-muted">{spec}</span>
                  </div>
                ))}
              </div>
            </GlassCard>
        ...
        <GlassCard className="p-8 border-crypto-accent/20 bg-gradient-to-br from-crypto-accent/5 via-crypto-card/50 to-crypto-accent2/5">
          <div className="flex items-start gap-4">
            <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-crypto-accent to-crypto-accent3 flex items-center justify-center shadow-xl shadow-crypto-accent/20">
            ...
            <div className="flex-1">
              ...
              <div className="grid grid-cols-4 gap-3">
                {[
                  { name: 'Transformer', weight: '35%', color: 'from-violet-500 to-purple-600' },
                  ...
                  <div key={m.name} className="p-3 rounded-xl bg-crypto-bg/50 border border-crypto-border/30 text-center">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <GlassCard className="p-6">
            ...
            <p className="text-sm text-crypto-muted leading-relaxed mb-4">Lexicon VADER-like (bullish/bearish keywords) + optional FinBERT transformer. Sources: CryptoPanic news, Reddit r/CryptoCurrency, CoinGecko trending. Daily aggregated compound score merged into price DataFrame.</p>
            <div className="space-y-2 text-xs mono">
              <div className="p-2 rounded-lg bg-crypto-bg border border-crypto-border">Sentiment_Compound: -1 to 1</div>
              <div className="p-2 rounded-lg bg-crypto-bg border border-crypto-border">Sentiment_MA7: 7-day moving avg</div>
              <div className="p-2 rounded-lg bg-crypto-bg border border-crypto-border">Sentiment_Count: mention volume</div>
            </div>
          </GlassCard>
          ...
          <GlassCard className="p-6">
            ...
            <p className="text-sm text-crypto-muted leading-relaxed mb-4">69 technical indicators: SMA, EMA, RSI, MACD, Bollinger Bands, ATR, lag features, volume features, price features. Sequence length 60 for LSTM/Transformer.</p>
            <div className="flex flex-wrap gap-1.5">
              {['SMA 7/14/30/50', 'EMA 12/26/50', 'RSI 14', 'MACD', 'BB 20', 'ATR 14', 'Lag 1/3/7/14', 'Volume'].map(f => (
                <span key={f} className="px-2 py-1 rounded-full bg-crypto-card border border-crypto-border text-[11px] text-crypto-muted">{f}</span>
              ))}
            </div>
          </GlassCard>
          ...
          <GlassCard className="p-6">
            <h3 className="font-bold text-white mb-4 flex items-center gap-2">
              <GitBranch size={16} className="text-crypto-accent2" />
              Training Pipeline
            </h3>
            <p className="text-sm text-crypto-muted leading-relaxed mb-4">TimeSeriesSplit 5 folds, early stopping, checkpointing, MLflow optional. Saves models to /models. Supports incremental training with new data.</p>
            <div className="space-y-2">
              <div className="flex justify-between text-xs p-2 rounded-lg bg-crypto-bg border border-crypto-border">
                <span className="text-crypto-muted">Test Size</span>
                <span className="text-white font-bold">20%</span>
              </div>
              <div className="flex justify-between text-xs p-2 rounded-lg bg-crypto-bg border border-crypto-border">
                <span className="text-crypto-muted">Val Size</span>
                <span className="text-white font-bold">10%</span>
              </div>
              <div className="flex justify-between text-xs p-2 rounded-lg bg-crypto-bg border border-crypto-border">
                <span className="text-crypto-muted">Sequence</span>
        ...
        <GlassCard className="p-6">
          <h3 className="font-bold text-white mb-4 flex items-center gap-2">
            <Cpu size={16} className="text-crypto-accent" />
            Realtime & Backtesting
          </h3>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              <div className="text-sm font-bold text-white mb-2">Binance Realtime</div>
              <p className="text-xs text-crypto-muted leading-relaxed">WebSocket wss://stream.binance.com:9443/ws/btcusdt@trade + kline_1m. Deque buffer 1000, threading, auto-reconnect 5s. LivePredictor merges live price with model forecast. REST fallback for 24h ticker and klines.</p>
              <div className="mt-3 p-2 rounded-lg bg-crypto-bg border border-crypto-border mono text-[11px] text-crypto-muted">wss://stream.binance.com:9443/stream?streams=btcusdt@ticker/ethusdt@ticker/...</div>
            </div>
            <div>
              <div className="text-sm font-bold text-white mb-2">Backtesting</div>
              <p className="text-xs text-crypto-muted leading-relaxed">Long-only simulation: initial capital $10k, commission 0.1%, slippage 0.05%. Strategies: MA crossover (20/50), RSI (30/70), Prediction threshold, Ensemble (pred+sentiment+RSI). Metrics: total return, Sharpe, max DD, win rate, profit factor.</p>
              <div className="mt-3 flex gap-2">
                <span className="px-2 py-1 rounded-full bg-crypto-card border border-crypto-border text-[11px] text-crypto-muted">Sharpe</span>
                <span className="px-2 py-1 rounded-full bg-crypto-card border border-crypto-border text-[11px] text-crypto-muted">Max DD</span>
                <span className="px-2 py-1 rounded-full bg-crypto-card border border-crypto-border text-[11px] text-crypto-muted">Win Rate</span>
              </div>
            </div>
          </div>
        </GlassCard>
