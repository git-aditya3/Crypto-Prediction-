        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
          <div>
            <h1 className="text-3xl font-black tracking-tight flex items-center gap-3">
              ...
              <span className="text-[var(--text)]">Live Trading</span>
              <span className="px-3 py-1 rounded-full bg-[var(--sell)]/10 border border-crypto-bear/20 text-[var(--sell)] text-xs font-black tracking-widest animate-pulse flex items-center gap-2">
                <span className="w-2 h-2 bg-[var(--sell)] rounded-full animate-ping"></span>
                LIVE • BINANCE WS
              </span>
            </h1>
          ...
          <div className="flex items-center gap-3">
            <select 
              ...
              className="bg-[var(--card)] border border-[var(--border)] rounded-xl px-4 py-2.5 text-sm font-medium text-[var(--text)] focus:border-crypto-accent/50 focus:outline-none"
            >
            ...
            <button 
              ...
              className={`px-5 py-2.5 rounded-xl font-bold text-sm transition-all flex items-center gap-2 ${
                isLive 
                  ? 'bg-[var(--sell)] text-[var(--text)] hover:bg-red-600 shadow-lg shadow-crypto-bear/20' 
                  : 'bg-[var(--buy)] text-black hover:bg-emerald-400 shadow-lg shadow-crypto-bull/20'
              }`}
            >
              <div className={`w-2 h-2 rounded-full ${isLive ? 'bg-white animate-pulse' : 'theme-bg'}`}></div>
              ...
              <div className="relative">
                <div className="flex items-start justify-between mb-8">
                  <div>
                    <div className="flex items-center gap-3 mb-2">
                      <h2 className="text-2xl font-black text-[var(--text)] tracking-tight">{selectedSymbol}</h2>
                      <span className="px-2.5 py-1 rounded-full bg-[var(--accent)]/10 border border-crypto-accent/20 text-[var(--accent)] text-xs font-bold">
                        {currentTicker?.binanceSymbol || selectedSymbol.replace('-','')}
                      </span>
                      <span className="flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-crypto-bg border border-[var(--border)] text-xs font-bold text-[var(--text)]">
                        <span className="live-dot !w-1.5 !h-1.5"></span>
                        LIVE
                      </span>
                    </div>
                    <div className="flex items-baseline gap-4">
                      <span className="text-5xl font-black mono tracking-tight text-[var(--text)]">
                      ...
                      <span className={`flex items-center gap-1.5 px-3 py-1 rounded-full text-sm font-bold ${
                        (currentTicker?.priceChangePercent || 0) >= 0 
                          ? 'bg-[var(--buy)]/10 text-[var(--buy)] border border-crypto-bull/20' 
                          : 'bg-[var(--sell)]/10 text-[var(--sell)] border border-crypto-bear/20'
                      }`}>
                    ...
                    <div className="text-sm text-[var(--text-muted)] mt-2">
                      24h Change: <span className={currentTicker?.priceChangePercent >= 0 ? 'text-[var(--buy)]' : 'text-[var(--sell)]'}>${safeFixed(currentTicker?.priceChange,2) || '0.00'}</span> • 
                      Volume: <span className="text-[var(--text)] font-medium">{(currentTicker?.volume ?? 0).toLocaleString()} {selectedSymbol.split('-')[0]}</span>
                    </div>
                  </div>

                  <div className="hidden md:block text-right">
                    <div className="text-xs font-bold tracking-widest text-[var(--text-muted)] uppercase mb-2">Live Signal</div>
                    {liveSignal ? (
                      <div className={`inline-flex items-center gap-2 px-4 py-2 rounded-xl font-black text-sm border ${
                        liveSignal.signal?.includes('BUY') 
                          ? 'bg-[var(--buy)]/10 text-[var(--buy)] border-crypto-bull/20' 
                          : liveSignal.signal?.includes('SELL')
                            ? 'bg-[var(--sell)]/10 text-[var(--sell)] border-crypto-bear/20'
                      ...
                      <div className="px-4 py-2 rounded-xl bg-[var(--card)] border border-[var(--border)] text-[var(--text-muted)] text-sm">
                        Calculating...
                      </div>
                    )}
                  </div>
                ...
                <div>
                  <div className="flex items-center justify-between mb-3">
                    <span className="text-xs font-bold tracking-widest text-[var(--text-muted)] uppercase">Live Price Action (Last 50 Trades)</span>
                    <span className="text-xs text-[var(--text-muted)]">{liveTrades.length} trades • Real-time</span>
                  </div>
                  <div className="h-24 flex items-end gap-0.5 p-3 rounded-xl bg-crypto-bg/50 border border-[var(--border)]/30">
                    {liveTrades.length > 0 ? (
                      liveTrades.slice(0, 50).reverse().map((trade, i) => {
                        ...
                        return (
                          <div
                            key={i}
                            className={`flex-1 rounded-t transition-all duration-300 ${trade.isBuyerMaker ? 'bg-[var(--sell)]/60' : 'bg-[var(--buy)]/60'} hover:opacity-80`}
                ...
                <div className="grid grid-cols-4 gap-3 mt-6">
                  {[
                    { label: 'High', value: currentTicker?.high, icon: TrendingUp, color: 'text-[var(--buy)]' },
                    { label: 'Low', value: currentTicker?.low, icon: TrendingDown, color: 'text-[var(--sell)]' },
                    { label: 'Open', value: currentTicker?.open, icon: Clock, color: 'text-[var(--text-muted)]' },
                    { label: 'Quote Vol', value: currentTicker?.quoteVolume ? `${safeFixed(currentTicker.quoteVolume/1e6,2)}M` : '—', icon: BarChart3, color: 'text-[var(--accent)]' },
                  ].map((stat, i) => (
                    <div key={i} className="p-3 rounded-xl bg-crypto-bg/40 border border-[var(--border)]/20">
                      <div className="flex items-center gap-1.5 mb-1">
                        <stat.icon size={10} className={stat.color} />
                        <span className="text-[10px] font-bold tracking-widest text-[var(--text-muted)] uppercase">{stat.label}</span>
                      </div>
                      <div className="mono font-bold text-sm text-[var(--text)]">
          ...
          <div className="lg:col-span-4 space-y-6">
            <GlassCard className="p-6">
              <h3 className="font-bold text-[var(--text)] flex items-center gap-2 mb-4">
                ...
                <span className="ml-auto text-[10px] px-2 py-1 rounded-full bg-[var(--card)] border border-[var(--border)] text-[var(--text-muted)]">LIVE</span>
              </h3>
              ...
              {orderBook ? (
                <div className="space-y-4">
                  {/* Asks */}
                  <div>
                    <div className="flex justify-between text-[10px] font-bold tracking-widest text-[var(--text-muted)] uppercase mb-2">
                    ...
                    <div className="space-y-1">
                      {orderBook.asks?.slice(0, 7).reverse().map((ask, i) => {
                        ...
                        return (
                          <div key={i} className="flex justify-between text-xs mono relative group hover:bg-[var(--sell)]/5 p-1 rounded transition">
                            <span className="text-[var(--sell)] font-medium">{(price ?? 0).toFixed(2)}</span>
                            <span className="text-[var(--text)]">{(qty ?? 0).toFixed(4)}</span>
                            <span className="text-[var(--text-muted)]">{((price ?? 0)*(qty ?? 0)).toFixed(2)}</span>
                            <div className="absolute inset-0 bg-[var(--sell)]/5 opacity-0 group-hover:opacity-100 rounded transition" style={{ width: `${Math.min(100, qty*10)}%`, right: 0, left: 'auto' }}></div>
                          </div>
                        )
                      })}
                    </div>
                  ...
                  <div className="py-2 flex items-center justify-center gap-2">
                    ...
                    <span className="text-xs font-black mono text-[var(--accent)] px-3 py-1 rounded-full bg-[var(--accent)]/10 border border-crypto-accent/20">
                  ...
                  <div className="space-y-1">
                    {orderBook.bids?.slice(0, 7).map((bid, i) => {
                      ...
                      return (
                        <div key={i} className="flex justify-between text-xs mono relative group hover:bg-[var(--buy)]/5 p-1 rounded transition">
                          <span className="text-[var(--buy)] font-medium">{(price ?? 0).toFixed(2)}</span>
                          <span className="text-[var(--text)]">{(qty ?? 0).toFixed(4)}</span>
                          <span className="text-[var(--text-muted)]">{((price ?? 0)*(qty ?? 0)).toFixed(2)}</span>
                        </div>
                      )
            ...
            <GlassCard className="p-6">
              ...
              <div className="space-y-1 max-h-[300px] overflow-y-auto">
                {trades.length > 0 ? trades.slice(0, 15).map((trade, i) => {
                  ...
                  return (
                    <div key={i} className="flex justify-between items-center text-xs mono p-2 rounded-lg hover:bg-[var(--card)]/50 transition">
                      <span className={isBuy ? 'text-[var(--buy)]' : 'text-[var(--sell)]'}>{parseFloat(trade.price ?? 0).toFixed(2)}</span>
                      <span className="text-[var(--text)]">{parseFloat(trade.qty ?? 0).toFixed(4)}</span>
                      <span className="text-[var(--text-muted)] text-[11px]">{new Date(trade.time).toLocaleTimeString()}</span>
                    </div>
                  )
                }) : liveTrades.slice(0, 15).map((trade, i) => (
                  <div key={i} className="flex justify-between items-center text-xs mono p-2 rounded-lg hover:bg-[var(--card)]/50 transition">
                    <span className={trade.isBuyerMaker ? 'text-[var(--sell)]' : 'text-[var(--buy)]'}>{safeFixed(trade.price,2)}</span>
                    <span className="text-[var(--text)]">{safeFixed(trade.qty,4)}</span>
                    <span className="text-[var(--text-muted)] text-[11px]">{new Date(trade.time).toLocaleTimeString()}</span>
                  </div>
                ))}
              </div>
        ...
        <GlassCard className="p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            {[
              ...
              <div key={item.step} className="p-4 rounded-xl bg-crypto-bg/40 border border-[var(--border)]/30 hover:border-[var(--border)]/60 transition group">
