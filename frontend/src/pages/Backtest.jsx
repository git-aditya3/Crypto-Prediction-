          ...
          <div>
            <h1 className="text-3xl font-black tracking-tight flex items-center gap-3">
              <span className="w-10 h-10 rounded-xl bg-gradient-to-br from-crypto-accent2 to-purple-600 flex items-center justify-center shadow-lg">
                <BarChart3 size={20} className="text-[var(--text)]" />
              </span>
              <span className="text-[var(--text)]">Backtesting Engine</span>
              <span className="px-3 py-1 rounded-full bg-zinc-100 dark:bg-zinc-800 border border-black/5 dark:border-white/5 text-zinc-900 dark:text-[var(--text)] text-xs font-bold tracking-widest">PORTFOLIO SIM</span>
            </h1>
        ...
        <GlassCard className="p-6">
          <div className="flex flex-wrap gap-4 items-end">
            <div>
              ...
              <select value={selectedSymbol} onChange={e => setSelectedSymbol(e.target.value)} className="block mt-1 bg-transparent border border-black/5 dark:border-white/5 rounded-xl px-4 py-2.5 text-sm font-medium text-[var(--text)] min-w-[140px]">
            ...
            <div>
              ...
              <select value={strategy} onChange={e => setStrategy(e.target.value)} className="block mt-1 bg-transparent border border-black/5 dark:border-white/5 rounded-xl px-4 py-2.5 text-sm font-medium text-[var(--text)] min-w-[200px]">
          ...
          <>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
              <div className="lg:col-span-8">
                <GlassCard className="p-6">
                  <h3 className="font-bold text-[var(--text)] mb-4 flex items-center gap-2">
                    <Activity size={18} className="text-zinc-900 dark:text-[var(--text)]" />
              ...
              <div className="lg:col-span-4 space-y-6">
                <GlassCard className="p-6">
                  <h3 className="font-bold text-[var(--text)] mb-4">Detailed Metrics</h3>
                  <div className="space-y-2.5">
                    {Object.entries(result.metrics).map(([k, v]) => (
                      <div key={k} className="flex justify-between items-center p-2.5 rounded-xl bg-transparent/40 border border-black/5 dark:border-white/5/20 hover:border-black/5 dark:border-white/5/40 transition">
                        ...
                        <span className="mono text-xs font-bold text-[var(--text)]">{typeof v === 'number' ? v.toFixed(2) : v}</span>
                      </div>
                    ))}
                  </div>
                </GlassCard>
                ...
                <GlassCard className="p-6">
                  <h3 className="font-bold text-[var(--text)] mb-4">Recent Trades ({result.trades.length})</h3>
                  <div className="space-y-2 max-h-[300px] overflow-y-auto">
                    {result.trades.slice(-10).reverse().map((t, i) => (
                      <div key={i} className="flex items-center justify-between p-3 rounded-xl bg-transparent/40 border border-black/5 dark:border-white/5/20">
                        <div>
                          <div className="text-xs font-bold text-[var(--text)]">{t.entry_date?.slice(0,10)} → {t.exit_date?.slice(0,10)}</div>
        ...
        {compare && (
          <GlassCard className="p-6">
            <h3 className="font-bold text-[var(--text)] mb-6 flex items-center gap-2">
              <BarChart3 size={18} className="text-zinc-900 dark:text-[var(--text)]" />
                ...
                <tbody>
                  {Object.entries(compare).map(([name, m]) => (
                    <tr key={name} className="border-b border-black/5 dark:border-white/5/20 hover:ui-card/30 transition">
                      <td className="p-3 font-bold text-[var(--text)]">{name}</td>
                      ...
                      <td className="p-3 text-right mono text-[var(--text)]">{safeFixed(m.sharpe_ratio,2)}</td>
                      <td className="p-3 text-right mono text-red-500">{safeFixed(m.max_drawdown_pct,2)}%</td>
                      <td className="p-3 text-right mono text-[var(--text)]">{safeFixed(m.win_rate_pct,1)}%</td>
                      <td className="p-3 text-right mono text-zinc-500">{m.num_trades}</td>
                      <td className="p-3 text-right mono text-[var(--text)]">{m.profit_factor ? safeFixed(m.profit_factor,2) : "—" || '—'}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
        ...
        {!result && !compare && (
          <GlassCard className="p-12 text-center">
            <div className="w-20 h-20 mx-auto mb-4 rounded-2xl bg-gradient-to-br from-crypto-accent2/20 to-purple-500/20 border border-crypto-accent2/20 flex items-center justify-center">
              <BarChart3 size={32} className="text-zinc-900 dark:text-[var(--text)]" />
            </div>
            <h3 className="font-bold text-[var(--text)] text-lg mb-2">Backtesting Engine Ready</h3>
