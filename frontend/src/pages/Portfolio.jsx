      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-black flex items-center gap-3">
            <Wallet className="text-emerald-600" /> Real Portfolio - CoinDCX
            ...
            <span className="px-3 py-1 rounded-full bg-red-500/10 border border-red-500/10 text-red-500 text-sm font-bold">ACTUAL INR</span>
          </h1>
          <p className="text-zinc-500 mt-1">Real holdings tracking - CoinDCX actual INR and crypto - no paper simulation - real P&L</p>
        </div>
          ...
          <div className="space-y-2">
            {Object.entries(allocation).map(([sym, pct]) => (
              <div key={sym} className="flex items-center gap-3">
                ...
                <div className="w-12 text-xs text-zinc-500">{(pct ?? 0).toFixed(1)}%</div>
              </div>
            ))}
          </div>
        </div>
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {openPositions.map((p, i) => (
                  <tr key={i} className="border-t border-black/5 dark:border-white/5/30">
                    <td className="py-3 font-bold">{p.symbol || 'Unknown'}</td>
                    <td><span className={`px-2 py-0.5 rounded-full text-xs font-bold ${p.side === 'LONG' ? 'bg-zinc-100 dark:bg-zinc-800 text-emerald-600' : 'bg-red-500/10 text-red-500'}`}>{p.side || 'LONG'}</span></td>
                    <td>${(p.entry_price ?? 0).toLocaleString(undefined, { maximumFractionDigits: 2 })}</td>
                    <td>{(p.quantity ?? 0).toFixed(6)}</td>
                    <td className="text-red-500">${(p.stop_loss ?? 0).toLocaleString(undefined, { maximumFractionDigits: 2 })}</td>
                    <td className="text-emerald-600">${(p.take_profits?.tp1 ?? 0).toLocaleString(undefined, { maximumFractionDigits: 2 })}</td>
                    <td>{p.leverage || '1x'}</td>
                    <td><button onClick={() => closePos(p.symbol)} className="px-3 py-1 rounded-lg bg-red-500/10 text-red-500 hover:bg-red-500/20 text-xs">Close Real</button></td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <div className="p-5 rounded-2xl ui-card border border-black/5 dark:border-white/5">
          ...
          <div className="space-y-2 max-h-[300px] overflow-auto">
            {closedPositions.slice(-20).reverse().map((p, i) => (
              <div key={i} className="flex items-center justify-between p-3 rounded-xl bg-transparent border border-black/5 dark:border-white/5/50">
                <div className="flex items-center gap-3">
                  <span className="font-bold">{p.symbol}</span>
                  <span className={`text-xs px-2 py-0.5 rounded-full ${p.side === 'LONG' ? 'bg-zinc-100 dark:bg-zinc-800 text-emerald-600' : 'bg-red-500/10 text-red-500'}`}>{p.side}</span>
                  <span className="text-xs text-zinc-500">{(p.quantity ?? 0).toFixed(6)} @ ${(p.entry_price ?? 0).toFixed(2)}</span>
                </div>
                <div className={`font-bold ${(p.pnl ?? 0) >= 0 ? 'text-emerald-600' : 'text-red-500'}`}>${(p.pnl ?? 0).toFixed(2)} ({(p.pnl_pct ?? 0).toFixed(2)}%)</div>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur p-4">
          <div className="p-6 rounded-2xl ui-card border border-black/5 dark:border-white/5 w-full max-w-md">
            ...
            <div className="space-y-4">
              <div>
                <label className="text-xs text-zinc-500">Symbol - CoinDCX Market (BTCINR etc)</label>
                <select value={form.symbol} onChange={e => setForm({ ...form, symbol: e.target.value })} className="w-full mt-1 px-3 py-2 rounded-xl bg-transparent border border-black/5 dark:border-white/5">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <select value={form.side} onChange={e => setForm({ ...form, side: e.target.value })} className="w-full mt-1 px-3 py-2 rounded-xl bg-transparent border border-black/5 dark:border-white/5">
                ...
                <div>
                  ...
                  <input type="number" step="0.000001" value={form.quantity} onChange={e => setForm({ ...form, quantity: e.target.value })} className="w-full mt-1 px-3 py-2 rounded-xl bg-transparent border border-black/5 dark:border-white/5" />
                </div>
              </div>
              <div>
                <label className="text-xs text-zinc-500">Entry Price (leave empty for live CoinDCX price)</label>
                <input type="number" value={form.entry_price} onChange={e => setForm({ ...form, entry_price: e.target.value })} placeholder="Live CoinDCX price" className="w-full mt-1 px-3 py-2 rounded-xl bg-transparent border border-black/5 dark:border-white/5" />
              </div>
              <div className="p-3 rounded-xl bg-emerald-500/5 border border-black/5 dark:border-white/5 text-xs">
                ...
                <div className="text-zinc-500 mt-1">This uses ACTUAL INR from your CoinDCX account - real trades, real P&L as you requested. Uses live CoinDCX price BTCINR etc. Risk guards protect capital. No paper simulation.</div>
              </div>
