  ...
  return (
    <div className="max-w-[1600px] mx-auto px-6 py-6 font-poppins space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h1 className="text-3xl font-black flex items-center gap-3"><Bell className="text-zinc-500" /> Alerts - Real Market Monitoring</h1>
              ...
              <div key={alert.id} className={`p-4 rounded-xl border flex items-center justify-between ${alert.status === 'TRIGGERED' ? 'bg-amber-500/5 border-amber-500/20' : alert.status === 'ACTIVE' ? 'bg-transparent border-black/5 dark:border-white/5/50' : 'bg-transparent/50 border-black/5 dark:border-white/5/20 opacity-60'}`}>
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <div className="text-[10px] text-zinc-500 mt-0.5">Created: {alert.created_at ? new Date(alert.created_at).toLocaleString() : 'Unknown'} {alert.triggered_at ? `• Triggered: ${new Date(alert.triggered_at).toLocaleString()}` : ''}</div>
                  </div>
                </div>
                <button onClick={() => deleteAlert(alert.id)} className="p-2 rounded-lg hover:bg-red-500/10 text-zinc-500 hover:text-red-500"><Trash2 size={16} /></button>
              </div>
            ))}
          </div>
        )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur p-4">
          <div className="p-6 rounded-2xl ui-card border border-black/5 dark:border-white/5 w-full max-w-md">
            ...
            <div className="space-y-4">
              ...
              <select value={form.type} onChange={e => setForm({ ...form, type: e.target.value })} className="w-full px-3 py-2 rounded-xl bg-transparent border border-black/5 dark:border-white/5">
              ...
              <input type="number" placeholder="Target Price (optional for signal alerts)" value={form.target_price} onChange={e => setForm({ ...form, target_price: e.target.value })} className="w-full px-3 py-2 rounded-xl bg-transparent border border-black/5 dark:border-white/5" />
              <div className="p-3 rounded-xl bg-amber-500/5 border border-amber-500/20 text-xs">
                <div className="font-bold text-zinc-500">Real Market Alert</div>
                <div className="text-zinc-500 mt-1">Monitors live Binance price - triggers when condition met for real trading</div>
              </div>
              <div className="flex gap-2">
                <button onClick={() => setShowCreate(false)} className="flex-1 py-2 rounded-xl bg-transparent border border-black/5 dark:border-white/5">Cancel</button>
