...
export default function GlassCard({ children, className = '', hover = true, glow = false, variant = 'default', style = {}, ...props }) {
  ...
  return (
    <div 
      className={`${variantClasses[variant] || variantClasses.default} ${hover ? 'hover-lift' : ''} ${blur ? 'glass' : ''} ${glow ? 'shadow-[var(--glow)]' : ''} ${className} gpu-accelerated`}
      style={{ ...style }}
      {...props}
    >
...
export function StatCard({ label, value, subValue, trend, icon: Icon, accent = false }) {
  ...
  return (
    <GlassCard className={`p-5 group ${accent ? 'border-[var(--accent)] shadow-[var(--glow)]' : ''}`} hover={true} glow={accent}>
      <div className="flex items-start justify-between mb-4 gpu-accelerated">
        <div className="flex items-center gap-2">
          ...
          {accent && <div className="w-1.5 h-1.5 rounded-full bg-[var(--accent)] animate-pulse" />}
        </div>
        {Icon && (
          <div className={`w-8 h-8 rounded-lg flex items-center justify-center transition-transform duration-200 ease-out group-hover:scale-110 group-hover:rotate-3 gpu-accelerated ${accent ? 'bg-[var(--accent)] text-white shadow-[var(--glow)]' : 'bg-[var(--accent-soft)] text-[var(--accent)] border border-[var(--border)]'}`}>
            <Icon size={14} />
          </div>
        )}
      </div>
      <div className="space-y-2">
        ...
        {subValue && (
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {trend && (
              <span className={`px-2 py-0.5 rounded-full text-[10px] font-medium flex items-center gap-1 border gpu-accelerated transition-all duration-200 ${isPositive ? 'bg-[var(--buy)] text-white border-[var(--buy)] shadow-sm' : isNegative ? 'bg-[var(--sell-soft)] text-[var(--sell)] border-[var(--sell-border)]' : 'bg-[var(--bg-secondary)] text-[var(--text-sec)] border-[var(--border)]'}`}>
  ...
  return (
    <GlassCard className={`p-5 group ${className}`} hover={true}>
      <div className="flex items-start gap-3.5">
        {Icon && (
          <div className="w-9 h-9 rounded-lg flex items-center justify-center shrink-0 bg-[var(--accent)] text-white shadow-sm transition-transform duration-200 ease-out group-hover:scale-110 group-hover:rotate-3 gpu-accelerated">
            <Icon size={16} />
          </div>
        )}
...
export function MetricCard({ label, value, change, icon: Icon, className = '' }) {
  ...
  return (
    <div className={`p-3.5 rounded-xl border bg-[var(--card)] border-[var(--border)] hover:border-[var(--border-strong)] transition-all duration-200 ease-out hover:translate-y-[-1px] hover:shadow-[var(--shadow-md)] group gpu-accelerated ${className}`}>
      ...
      {change && (
        <div className={`text-[11px] mt-1 flex items-center gap-1 font-medium ${isPositive ? 'text-[var(--buy)]' : 'text-[var(--sell)]'}`}>
