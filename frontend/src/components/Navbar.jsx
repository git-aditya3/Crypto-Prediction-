...
import { TrendingUp, Brain, Radio, BarChart3, Home, Sparkles, Activity, Zap, Menu, X, Target, Settings } from 'lucide-react'
...
export default function Navbar() {
  ...
  const nav = [
    ...
    { path: '/trading', label: 'Trading Calls', icon: Target, desc: 'Professional signals', badge: 'PRO', highlight: true },
    { path: '/forecast', label: 'Forecast', icon: TrendingUp, desc: 'AI predictions' },
    { path: '/realtime', label: 'Live', icon: Radio, desc: 'Real-time feed', badge: 'LIVE' },
  ...
  return (
    <nav className="sticky top-0 z-50 backdrop-blur-2xl bg-crypto-bg/70 border-b border-crypto-border/50">
      <div className="absolute inset-0 bg-gradient-to-r from-crypto-accent/5 via-transparent to-emerald-500/5 pointer-events-none"></div>
      
      <div className="relative max-w-[1600px] mx-auto px-6 py-3.5 flex items-center justify-between">
        {/* Logo */}
        <Link to="/" className="flex items-center gap-3 group">
          <div className="relative">
            <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-crypto-accent to-emerald-500 flex items-center justify-center font-black text-black text-lg shadow-lg shadow-crypto-accent/20 group-hover:shadow-crypto-accent/30 transition-all duration-300 group-hover:scale-105">
              ₿
            </div>
          ...
          <div className="hidden sm:block">
            <div className="font-bold text-[17px] tracking-tight leading-none flex items-center gap-2">
              CryptoPred
              <span className="px-2 py-0.5 rounded-full bg-gradient-to-r from-emerald-500/20 to-crypto-accent/20 border border-emerald-500/20 text-emerald-400 text-[10px] font-bold tracking-widest">V3 • TRADING</span>
            </div>
            <div className="text-[11px] text-crypto-muted font-medium tracking-wide mt-0.5 flex items-center gap-2">
              <span>AI • TRADING CALLS • REAL-TIME</span>
              {isLive && (
          ...
          {nav.map(item => {
            ...
            return (
              <Link 
                key={item.path} 
                to={item.path} 
                className={`relative flex items-center gap-2 px-3.5 py-2.5 rounded-xl text-[13px] font-medium transition-all duration-300 group ${
                  active 
              ...
              >
                <item.icon size={15} className={`${active ? 'text-black' : item.highlight ? 'text-emerald-400 group-hover:text-white' : 'group-hover:text-white'} transition-colors`} />
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="hidden md:flex items-center gap-2">
            <Link to="/trading" className="px-3 py-2 rounded-xl bg-gradient-to-r from-emerald-500/10 to-crypto-accent/10 border border-emerald-500/20 hover:border-emerald-500/40 transition">
              <div className="flex items-center gap-2 text-xs">
                <Target size={12} className="text-emerald-400" />
                <span className="font-semibold text-emerald-400">ARIMA 2.57% • Live Calls</span>
              </div>
            </Link>
          </div>
              ...
              return (
                <Link
                  ...
                  className={`p-4 rounded-xl border transition ${
                    ...
                    item.highlight ? 'bg-emerald-500/10 border-emerald-500/20 text-emerald-400 hover:bg-emerald-500/20' :
                ...
                >
                  ...
                  <div className="font-semibold text-sm flex items-center gap-2">
                    {item.label}
                    {item.badge && <span className="text-[9px] px-1.5 py-0.5 rounded-full bg-emerald-500 text-black font-black">{item.badge}</span>}
                  </div>
